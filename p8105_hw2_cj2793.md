...
Here we can see that 1 records from the results_df that do not have a
matching “Series” and “Baker” in the bakers_df; 8 records from the
...
Here we can see that 589 records from the bakers_results that do not
...
After the first step of merging bakers.csv with the cleaned results.csv
file, the second merge was performed with bakes.csv, using both Series,
Episode, and Baker to match contestants’ demographic information to
...
```
    ...
    ##    Episode Series.1 Series.2 Series.3 Series.4 Series.5 Series.6 Series.7
