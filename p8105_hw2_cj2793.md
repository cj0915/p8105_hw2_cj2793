...
were stored as numeric values, while in others, they were character
values. To standardize the data, these columns were converted to
character type across all datasets, ensuring that the merging process
...
To resolve this, the first names were extracted from the Baker column in
bakers.csv using the mutate() and word() functions. This allowed the
datasets to be merged using the correct identifier.

After the first step of merging bakes.csv with the cleaned results.csv
file, the second merge was performed with bakers.csv, using both Series
and Baker to match contestants’ demographic information to their
performance data.
