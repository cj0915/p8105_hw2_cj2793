...
Let's check how many records from the results_df that do not have a matching "Series", "Episode", and "Baker" in the bakes_df and vice versa.
...
Here we can see that 596 records from the results_df that do not have a matching "Series", "Episode", and "Baker" in the bakes_df; 8 records from the bakes_df that do not have a matching "Series", "Episode", and "Baker" in the results_df.
...
We notice that in bakers_df, bakers have full name recorded. However, in bakers_df,
...
Then we can check how many records from the bakes_results that do not have a matching "Series" and "Baker" in the bakers_df and vice versa.
...
Here we can see that 8 records from the bakes_results that do not have a matching "Series", and "Baker" in the bakers_df; 26 records from the bakers_df that do not have a matching "Series" and "Baker" in the bakes_results.

## Merge and organize a single, final dataset

Finally, we can merge bakes_results and bakers_df to get the final dataset and rort rows by Series, Episode, and Baker to ensure meaningful order of observations.
...
Another issue encountered was the inconsistency in data types for the Series and Episode columns across the datasets. In some files, these were stored as numeric values, while in others, they were character values. To standardize the data, these columns were converted to character type across all datasets, ensuring that the merging process could proceed smoothly. One of the more significant challenges arose from the fact that the Baker column in bakes.csv and results.csv contained only first names, while in bakers.csv, full names were used. To resolve this, the first names were extracted from the Baker column in bakers.csv using the mutate() and word() functions. This allowed the datasets to be merged using the correct identifier.

After the first step of merging bakes.csv with the cleaned results.csv file, the second merge was performed with bakers.csv, using both Series and Baker to match contestants' demographic information to their performance data.
